...
COPY . . 
RUN GOPROXY="https://goproxy.io" GO111MODULE=on CGO_ENABLED=1 go build -o livetv .

FROM alpine:latest
RUN set -ex \
    && apk --no-cache add ca-certificates tzdata libc6-compat libgcc ffmpeg libstdc++ python3 \
